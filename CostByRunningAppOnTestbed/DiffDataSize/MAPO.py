...
Computation = Component200RPi + Component500VM + Component200VM + Component1000VM + Component2000RPi + (CompTime200RPi*Cost_Processing_GWRPi) + (CompTime200RPi*Cost_Processing_GWRPi) 
Cost65KB =  Computation + (Cost_Storage_RPi*F65KB )  + (Cost_Storage_GWRPi*F65KB *2) + (Cost_Storage_VM*F65KB  * 4) +   (CommTimeToVM65KB  * Cost_BW_VM) 
Cost125KB = Computation + (Cost_Storage_RPi*F125KB ) + (Cost_Storage_GWRPi*F125KB *2) + (Cost_Storage_VM*F125KB * 4) +   (CommTimeToVM125KB * Cost_BW_VM) 
Cost500KB = Computation + (Cost_Storage_RPi*F500KB) +  (Cost_Storage_GWRPi*F500KB *2) + (Cost_Storage_VM*F500KB * 4) +   (CommTimeToVM500KB * Cost_BW_VM) 
Cost1MB =   Computation + (Cost_Storage_RPi*F1MB)   +  (Cost_Storage_GWRPi*F1MB *2) + (Cost_Storage_VM*F1MB   * 4) +   (CommTimeToVM1MB   * Cost_BW_VM)
