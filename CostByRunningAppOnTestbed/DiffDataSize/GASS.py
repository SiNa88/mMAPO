...
Computation = (CompTime200RPi*Cost_Processing_GWRPi) + (CompTime500RPi*Cost_Processing_GWRPi) + (CompTime200RPi*Cost_Processing_GWRPi) + Component1000RPi + Component2000RPi + (CompTime200RPi*Cost_Processing_GWRPi) + (CompTime200RPi*Cost_Processing_GWRPi) 


Cost65KB =  Computation + (Cost_Storage_RPi*F65KB * 2) +  (Cost_Storage_GWRPi*F65KB* 5)+ (CommTimeToRPi65KB  * Cost_BW_RPi) + (CommTimeToRPi65KB  * Cost_BW_GWRPi) 
Cost125KB = Computation + (Cost_Storage_RPi*F125KB * 2) + (Cost_Storage_GWRPi*F65KB* 5)+ (CommTimeToRPi125KB * Cost_BW_RPi) + (CommTimeToRPi65KB  * Cost_BW_GWRPi)
Cost500KB = Computation + (Cost_Storage_RPi*F500KB * 2) + (Cost_Storage_GWRPi*F65KB* 5)+ (CommTimeToRPi500KB * Cost_BW_RPi) + (CommTimeToRPi65KB  * Cost_BW_GWRPi)
Cost1MB =   Computation + (Cost_Storage_RPi*F1MB * 2) +   (Cost_Storage_GWRPi*F65KB* 5)+ (CommTimeToRPi1MB   * Cost_BW_RPi) + (CommTimeToRPi65KB  * Cost_BW_GWRPi)
